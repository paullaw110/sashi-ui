...
// Custom collision detection that prefers hour slots over day columns
const hourSlotCollision: CollisionDetection = (args) => {
  // First get all collisions using pointerWithin
  const pointerCollisions = pointerWithin(args);
  
  // Prioritize hour slots (YYYY-MM-DD-HH format, e.g., "2026-01-27-09")
  ...
  // Then try day columns (YYYY-MM-DD format) for untimed section
...
function TimeGridTask({ 
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Get the grid height (600px min-height)
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        className="absolute bottom-0 left-0 right-0 h-2 cursor-ns-resize opacity-0 group-hover:opacity-100 hover:bg-blue-500/30 transition-opacity"
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div 
        ref={setDayRef}
        className={cn(
          "px-0.5 py-1 border-b border-[#1a1a1a] space-y-0 bg-[#0c0c0c] min-h-[32px] max-h-[120px] overflow-y-auto",
          isDayOver && "bg-blue-500/10"
        )}
      >
...
export function CalendarView({ tasks, projects }: CalendarViewProps) {
  ...
  // Group tasks by date, separating timed vs all-day
  const tasksByDate = useMemo(() => {
    const map = new Map<string, { timed: Task[]; allDay: Task[] }>();
    tasks.forEach((task) => {
      if (task.dueDate) {
        const dateKey = format(new Date(task.dueDate), "yyyy-MM-dd");
        const existing = map.get(dateKey) || { timed: [], allDay: [] };
        if (task.dueTime) {
          existing.timed.push(task);
        } else {
          existing.allDay.push(task);
        }
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    // Check if dropped on an hour slot (YYYY-MM-DD-HH format)
    const hourSlotMatch = dropTarget.match(/^(\d{4}-\d{2}-\d{2})-(\d{2})$/);
    if (hourSlotMatch) {
      targetDate = hourSlotMatch[1];
      const hour = parseInt(hourSlotMatch[2], 10);
      targetTime = `${hour.toString().padStart(2, "0")}:00`;
    }
    // Check if dropped on day column (untimed section) - YYYY-MM-DD format
    else if (/^\d{4}-\d{2}-\d{2}$/.test(dropTarget)) {
      targetDate = dropTarget;
      targetTime = null; // Dropping on day column removes the time
    }
    // Check if dropped on another task - find its day
    else {
      for (const [dateKey, dayData] of tasksByDate.entries()) {
        const allTasks = [...dayData.timed, ...dayData.allDay];
        if (allTasks.some(t => t.id === dropTarget)) {
          targetDate = dateKey;
          break;
        }
      }
    ...
    // If we found a target, update the task
    if (targetDate) {
  ...
  const handleResize = useCallback(async (taskId: string, duration: number) => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="bg-[#111] rounded-lg border border-[#1a1a1a]">
          ...
          <div className="flex overflow-hidden">
            ...
            <div className="w-12 shrink-0 border-r border-[#1a1a1a] flex flex-col bg-[#0c0c0c]">
              {/* Empty header space */}
              <div className="h-[52px] border-b border-[#1a1a1a]" />
              {/* Hour labels */}
        ...
        <DragOverlay>
          {activeTask ? (
            <div className="text-[11px] px-2 py-1.5 rounded bg-[#222] text-[#f5f5f5] shadow-xl border border-[#333] cursor-grabbing">
              {activeTask.name}
            </div>
          ) : null}
        </DragOverlay>
